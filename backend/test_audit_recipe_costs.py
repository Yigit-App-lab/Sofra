...
class AuditTests(unittest.TestCase):
    def test_flags_implausibly_cheap_protein_recipe(self):
        flags = cost_flags({
            "title": "Dana Etli Güveç", "category": "Ana yemek",
            "cost_per_portion": 3.7, "cost_coverage": 0.8,
        })
        self.assertIn("main_course_protein_under_30_per_person", flags)

    def test_flags_low_coverage(self):
        flags = cost_flags({
            "title": "Dana Etli Güveç", "category": "Ana yemek",
            "cost_per_portion": 100, "cost_coverage": 0.4,
        })
    ...
    def test_accepts_plausible_well_covered_cost(self):
        self.assertEqual(
            cost_flags({
                "title": "Dana Etli Güveç", "category": "Ana yemek",
                "cost_per_portion": 163.88, "cost_coverage": 0.8,
            }),
            [],
        )
